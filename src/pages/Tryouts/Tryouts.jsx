  ...
  getAllTestPackages()
    .then((res) => {
      if (res?.data) {
        setPackages(res.data); // simpan sebagai object, bukan array
      } else {
        setPackages({});
        console.error("Expected object with 'data' array, but got:", res);
      }
    })
    .catch(() => {
            ...
            packages.length === 0 ? (
              ...
              packages.map((pkg) => (
                <ProductCard
                  ...
                  buttonText="Detail"
                  onButtonClick={() => navigate(`/tryoutsDetail/${pkg.id}`)}
                >
