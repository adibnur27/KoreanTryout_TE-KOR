...
const VocabularyDashboard = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || err.message || 'An error occurred during file upload.'
      );
    } finally {
  ...
  const handleDeleteVocabulary = async (vocabularyId) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || err.message || 'Failed to delete vocabulary.'
      );
    } finally {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <li key={category.id} className="mb-2 p-2 rounded hover:bg-gray-100">
                <span
                  ...
                  className={`cursor-pointer font-medium w-full block ${
                    selectedCategory?.id === category.id ? 'text-blue-600' : 'text-gray-700'
                  }`}
                >
                  {category.name}
                </span>
        ...
        <div className="md:col-span-2 p-4 border rounded shadow-sm">
          ...
          {!isFetchingVocabs && !selectedCategory && (
            <p className="text-gray-500">Select a category to view vocabularies.</p>
          )}
          {!isFetchingVocabs && selectedCategory && vocabularies.length === 0 && (
            <p className="text-gray-500">No vocabularies found for this category.</p>
          )}
